...
# @db_bp.route('/signup/submit/',methods=['POST'])
# def signup_db():
...
#             return redirect(url_for('db.userLogin'))
#     return login_decorator

# # ***************************** DECORATOR *************************************************

# # ***************************** LOGIN TO DB *************************************************
# @db_bp.route('/userlogin/submit/',methods=['POST'])
# def userlogin_db():
...
# @db_bp.route("/profileform/update/<int:id>/",methods=["POST"])
...
# @db_bp.route('/seenIT/hub/db/', methods=['POST'])
# @login_required
# def seenITHub_db():
...
# Create database operations blueprint
db_bp = Blueprint('db', __name__)
logger = logging.getLogger(__name__)
...
def signup_db():
    try:
        ...
        if fname != '' and lname != '' and username != '' and email != '' and phone != '' and pwd != '':
            if pwd != cpwd:
                return jsonify({'status':'danger', 'message':'Passwords do not match!'})
            elif terms != 'y':
                return jsonify({'status':'danger', 'message':'Kindly agree to terms'})
            elif check: 
                if check.username == username:
                    return jsonify({'status':'username', 'message':'Username already taken!'})
                if check.email == email:
                    return jsonify({'status':'email', 'message':'Email already taken!'})
                if check.phone == phone:
                    return jsonify({'status':'phone', 'message':'Phone Number already taken!'})
            else:
                terms = 'Agreed'
                hashed_pwd = generate_password_hash(pwd)
                insert_to_db = Users(
                ...
                if user_id:
                    return jsonify({'status':'success', 'message':'You have successfully signed up to SeenIT, kindly login to do more'})
        else:
            return jsonify({'status':'danger', 'message': 'Fields with * cannot be empty!'})
    ...
    try:
        ...
        if access != '' and pwd != '':
            ...
            if from_db:
                ...
                if confirm_pwd:
                    session['isonline'] = from_db.id
                    return jsonify({'status': 'success', 'message': url_for('user.profile_page', id=session['isonline'])})
                else:
                    return jsonify({'status':'danger', 'message':'Invalid Password!'})
            else:
                return jsonify({'status':'danger', 'message':f'{access} is not a registered account, kindly Sign Up'})
        else:
            return jsonify({'status':'empty', 'message':'Input fields cannot be empty!'})
            
    except Exception as e:
        logger.error(f"Login error: {str(e)}", exc_info=True)
        return jsonify({'status':'error', 'message':'An error occurred during login'})
# ***************************** LOGIN TO DB *************************************************
    ...
    try:
        ...
        if profile_pix and profile_pix.filename:
            ...
            if ext.lower() in format:
                ...
                return jsonify({'status':'profilepix', "msg":"Invalid image format!"})
        ...
        if fname == '' or lname == '':
            return jsonify({'status':'danger', 'msg':'First name and last name are required!'})

        if not user.profile:
            if state == '0':
                return jsonify({'status':'state', 'msg':'You need to pick your state of residence'})
            elif category == '0':
                return jsonify({'status':'category', 'msg':'You need to pick a category'})
            else:
                profile = Profile(
                    user_id=user.id,
            ...
            if not username:
                return jsonify({'status':'no_username', 'msg':'username cannot be empty!'})
            else:
                existing_user = db.session.query(Users).filter((Users.username==username), Users.id != id).first()
                if existing_user:
                    return jsonify({'status':'existing_user', 'msg':'username taken'})
                else:
                    user.username = username

            if not phone:
                return jsonify({'status':'no_phone', 'msg':'phone number cannot be empty!'})
            else:
                existing_phone = db.session.query(Users).filter((Users.phone==phone), Users.id != id).first()
                if existing_phone:
                    return jsonify({'status':'phone', 'msg':'phone number is taken'})
                else:
                    user.phone = phone
            ...
            if not gender:
                return jsonify({'status':'gender', 'msg':'Please select your gender'})
            ...
            if category == '0':
                return jsonify({'status':'category', 'msg':'You need to pick your category'})
            else:
                profile.category_id = int(category)

            if db_profile_pix_name: 
                profile.profile_pix = db_profile_pix_name
            if db_cover_pix_name:
                profile.cover_pix = db_cover_pix_name
            ...
            if services:
                ...
                return jsonify({'status':'services', 'msg':'You need to pick at least one service'})
            ...
            if skills:
                ...
                return jsonify({'status':'skills', 'msg':'You need to pick at least one skill'})

        db.session.commit()
        return jsonify({'status':'success', 'msg':'Your profile update was successful!'})
...
def seenITHub_db():
    try:
        ...
        db.session.rollback()
        logger.error(f"Project upload error: {str(e)}", exc_info=True)
        return jsonify({'status':'error', "msg":f"An error occurred: {str(e)}"})
# *********************************** SEENIT HUB PROJECT TO DB *************************************************
...
def contact_submit():
    try:
        ...
        if fname != '' and lname != '' and email != '' and message != '':
            ...
            if id:
                return jsonify({'status':'success', 'message': 'You have been added to our list'})
            else:
                return jsonify({'status':'danger', 'message': 'Error adding your message'})
        else:
            return jsonify({'status':'danger', 'message': 'Complete the fields!'})
...
def not_found(e):
    logger.error(f"DB 404 Error: {str(e)}", exc_info=True)
    return jsonify({'status':'error', 'message': 'Endpoint not found'}), 404

@db_bp.errorhandler(500)
def server_error(e):
    logger.error(f"DB 500 Error: {str(e)}", exc_info=True)
