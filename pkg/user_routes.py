...
def login_required(f):
    ...
    def login_decorator(*args, **kwargs):
        if session.get('isonline') != None:
            ...
            flash('You need to be logged in before you can visit page', category='danger')
...
# *********************************** SEENIT HUB PAGE **************************************************
@app.route('/seenIT/hub/', methods=['GET','POST'])
@login_required
def seenITHub():
...
def profile_page(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(f'An error occurred: {e}', 'error')
        return redirect(url_for('profile_page',id=session.get('isonline')))

# *********************************** PROFILE PAGE **************************************************

# *********************************** PROFILE FORM **************************************************
@app.route('/profileform/<int:id>/',methods=['GET','POST'])
@login_required
def profile_form(id):
